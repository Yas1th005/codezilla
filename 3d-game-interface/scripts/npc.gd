...
## NPC with proximity-based greetings and animation states
## Plays idle animation by default, talking animation when greeting player
...
@export var greeting_message: String = "Hello there, traveler!"
...
## First dialogue choice for the player
@export var choice1_text: String = "Hello! How are you today?"
## Second dialogue choice for the player
@export var choice2_text: String = "What do you do here?"
## Third dialogue choice for the player
@export var choice3_text: String = "Goodbye for now."

@export_group("NPC Responses")
## NPC response to choice 1
@export var response1_text: String = "I'm doing well, thank you for asking! The tavern has been busy today."
## NPC response to choice 2
@export var response2_text: String = "I'm the tavern keeper here. I serve drinks and provide rooms for weary travelers."
## NPC response to choice 3
@export var response3_text: String = "Farewell, traveler! Come back anytime you need rest or refreshment."
...
@export var idle_animation_name: String = "ZombieIdle(1)"
## Name of the thinking animation in the AnimationPlayer
@export var thinking_animation_name: String = "Thinking"
## Name of the talking animation in the AnimationPlayer
@export var talking_animation_name: String = "Talking(1)"
...
func setup_detection_area() -> void:
	...
	if detection_area and detection_collision:
		...
		print("NPC: Detection area configured as sphere with radius: ", detection_radius, " meters")
...
func setup_api_service() -> void:
	...
	if api_service and api_service.has_signal("dialogue_received"):
		...
		print("NPC: Connected to global API service")
	else:
		print("NPC Error: Could not find global API service")
...
func setup_audio_system() -> void:
	...
	print("NPC: Audio system initialized")
...
func start_idle_animation() -> void:
	...
	if animation_player and animation_player.has_animation(idle_animation_name):
		...
		print("NPC: Started idle animation: ", idle_animation_name)
	else:
		print("NPC Warning: Idle animation '", idle_animation_name, "' not found!")
		if animation_player:
			print("Available animations: ", animation_player.get_animation_list())

func start_thinking_animation() -> void:
	"""Start playing the thinking animation on loop"""
	if animation_player and animation_player.has_animation(thinking_animation_name):
		animation_player.play(thinking_animation_name, -1, 1.0, false)
		# Set the animation to loop
		var animation = animation_player.get_animation(thinking_animation_name)
		if animation:
			animation.loop_mode = Animation.LOOP_LINEAR
		print("NPC: Started thinking animation: ", thinking_animation_name)
	else:
		print("NPC Warning: Thinking animation '", thinking_animation_name, "' not found!")
		if animation_player:
			print("Available animations: ", animation_player.get_animation_list())
...
func start_talking_animation() -> void:
	...
	if animation_player and animation_player.has_animation(talking_animation_name):
		...
		if animation:
			animation.loop_mode = Animation.LOOP_LINEAR
		print("NPC: Started talking animation: ", talking_animation_name)
	else:
		print("NPC Warning: Talking animation '", talking_animation_name, "' not found!")
		if animation_player:
			print("Available animations: ", animation_player.get_animation_list())
...
func handle_thinking_timer(delta: float) -> void:
	...
	if is_thinking:
		...
		# Only auto-stop thinking if not waiting for API response
		if not waiting_for_api and thinking_timer >= 2.0:  # Think for 2 seconds
			stop_thinking()
...
func handle_face_player(delta: float) -> void:
	...
	elif not player_in_range and face_player:
		# Return to original rotation when player leaves
		var current_angle = rotation.y
		var angle_diff = angle_difference(original_rotation, current_angle)

		if abs(angle_diff) > 0.01:
			var rotation_step = rotation_speed * delta
			if abs(angle_diff) < rotation_step:
				rotation.y = original_rotation
			else:
				rotation.y += sign(angle_diff) * rotation_step
...
func update_animation_state() -> void:
	...
	elif is_thinking and animation_player.current_animation != thinking_animation_name:
		start_thinking_animation()
	elif not is_talking and not is_thinking and animation_player.current_animation != idle_animation_name:
		start_idle_animation()
...
func interact_with_player() -> void:
	...
	if can_interact and not is_talking and not is_thinking and not showing_choices and not waiting_for_api:
		...
		print("NPC: ", greeting_message)  # Keep for debugging
		is_talking = true
		talking_timer = 0.0
		start_talking_animation()
...
func handle_interaction_input() -> void:
	...
	if can_interact and Input.is_action_just_pressed("interact") and not showing_choices and not is_thinking and not waiting_for_api:
		print("NPC: E key pressed - starting interaction")
		interact_with_player()
	elif Input.is_action_just_pressed("interact"):
		# Debug why interaction isn't working
		print("NPC: E pressed but interaction blocked - can_interact:", can_interact, " showing_choices:", showing_choices, " is_thinking:", is_thinking, " waiting_for_api:", waiting_for_api)
...
func show_dialogue_choices() -> void:
	...
	print("NPC: Showing dialogue choices")
...
func handle_player_choice(choice_index: int, choice_text: String) -> void:
	...
	print("NPC: Player chose (index ", choice_index, "): '", choice_text, "'")

	# Start thinking animation while waiting for API response
	print("NPC: Starting thinking animation and API request")
	start_thinking()
	waiting_for_api = true

	# Send message to API
	if api_service and api_service.has_method("send_message"):
		print("NPC: Sending message to API service")
		api_service.send_message(choice_text)
	else:
		print("NPC Error: API service not available")
...
func stop_talking() -> void:
	...
	talking_timer = 0.0
	start_idle_animation()
	print("NPC: Finished talking, returning to idle")
...
func stop_thinking() -> void:
	...
	thinking_timer = 0.0
	start_idle_animation()
	print("NPC: Finished thinking, returning to idle")
...
func _on_dialogue_received(dialogue_data: Dictionary) -> void:
	...
	if dialogue.has("text"):
		...
		print("NPC responds (API): ", response_text)
		...
		if audio_base64 != "":
			...
			print("NPC: No audio base64 data provided")
...
func _on_api_error(error_message: String) -> void:
	...
	print("NPC API Error: ", error_message)
	...
	print("NPC responds (fallback): ", fallback_response)
...
func play_audio_from_base64(audio_base64: String) -> void:
	...
	print("NPC: Playing audio from base64 data (", audio_base64.length(), " characters)")
	...
	if audio_bytes.size() > 0:
		...
		print("NPC: Audio playing from base64 data")
	else:
		print("NPC Error: Failed to decode base64 audio data")



func _on_audio_finished() -> void:
	"""Called when audio playback finishes"""
	print("NPC: Audio playback finished")
...
func enable_interaction() -> void:
	...
	print("NPC: Interaction enabled - Press E to interact")
...
func disable_interaction() -> void:
	...
	print("NPC: Interaction disabled - player left range")
...
func _on_body_entered(body: Node3D) -> void:
	...
	if body.name == "ProtoController" or body.is_in_group("player"):
		...
		enable_interaction()
		print("NPC: Player entered 3m detection sphere - Distance: ", "%.2f" % distance, "m - can interact")
...
func _on_body_exited(body: Node3D) -> void:
	...
	if body.name == "ProtoController" or body.is_in_group("player"):
		...
		disable_interaction()
		print("NPC: Player left 3m detection sphere - Distance: ", "%.2f" % distance, "m")
